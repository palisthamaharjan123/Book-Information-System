...
//  AnimationController animationController;
  ...
  Widget _clippedV1(
      BuildContext context, double card_width, double card_height) {
    return new ClipPath(
  ...
  Widget _clippedNetwork(BuildContext context, double card_width,
    ...
    stackChildren.add(new Image.network(
      widget.imageUrl,
      width: card_width,
      height: card_height,
      fit: BoxFit.cover,
...
class StampClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    for (int i = 0; i < num / 2 - 1; i++) {
      ...
      path.relativeArcToPoint(new Offset(radius, 0.0),
          radius: new Radius.circular(radius / 2), clockwise: false);
    }
    path.relativeLineTo(radius, 0.0);
    ...
    for (int i = 0; i < numVert / 2 - 1; i++) {
      ...
      path.relativeArcToPoint(new Offset(0.0, radius),
          radius: new Radius.circular(radius / 2), clockwise: false);
    }
    path.relativeLineTo(0.0, radius);
